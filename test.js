...
async function getStatisticsOfErrors2(commits, baseUrl, login, token) {
    //TODO: work in progress
    let errors = new Map();
    for (const commit of commits) {
        ...
        if ('log' in commit) {
            ...
            while (log.includes(' ' + counter + ') ')) {
                // get the error message
                let error = log.slice(log.indexOf(' ' + counter + ') ') + (' ' + counter + ')').length, log.indexOf('\r\n', log.indexOf(' ' + counter + ') ')));
                // get the error message body
                //let error = log.slice(log.indexOf(' ' + counter + ') ') + (' ' + counter + ')').length, log.indexOf(":", log.indexOf(' ' + counter + ') ')));
                errorsCounter.set(error, 1);
                // save data about the error message in the map
                if (errors.has(error)) {
                    ...
                    errors.set(error, {all: 1, inBuilds: 0});
                }
                counter++;
            }
            for (const error of errorsCounter.keys()) {
...
async function main(baseUrl, pipelineName, login, token) {
    // get all commits for the pipeline
    ...
    let errors = await getStatisticsOfErrors2(result.flat(), baseUrl, login, token);
...
main("https://jenkins.mgmt.consoleconnect.com", "WP - api", "x-codete-b", "1123d0e44891879eae39c3d3e8556f2d10");
